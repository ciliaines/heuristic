...
def network_topology(Sources, Destinations):
    ...
    plt.axis('off')
    plt.title("Network  Topology", fontsize=10)
    nx.draw(G, with_labels=True,  node_size=400, font_size=10, edge_color='gray', width=1.0)   
...
def information_generator(Num_of_Frames, Streams_Period, Link_order_Descriptor, Network_links, Streams_links_paths,input_name):
    ...
    plt.text(0.1, 0.9, "Network links: " + str(Network_links), bbox=dict(facecolor='red', alpha=0.5), fontsize=10)
    plt.text(0.1, 0.75, "Frames per stream: " + str(Num_of_Frames), bbox=dict(facecolor='red', alpha=0.5), fontsize=10)
    plt.text(0.1, 0.6, "Stream periods: " + str(Streams_Period), bbox=dict(facecolor='red', alpha=0.5), fontsize=10)
    plt.text(0.1, 0.45, "Indexed link order per stream:  " + str(Link_order_Descriptor), bbox=dict(facecolor='red', alpha=0.5), fontsize=10)
    wrapped_text = "\n".join(textwrap.wrap(str(Streams_links_paths), width=120))
    plt.text(0.1, 0.25, "Stream paths:  " + str(wrapped_text), bbox=dict(facecolor='red', alpha=0.5), fontsize=10)
